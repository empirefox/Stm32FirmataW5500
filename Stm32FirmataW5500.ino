...
/*
 * Uncomment the #define SERIAL_DEBUG line below to receive serial output
 * messages relating to your
 * connection that may help in the event of connection issues. If defined, some
...
/*
 * Uncomment the following include to enable interfacing with Serial devices via
 * hardware or
 * software serial.
 *
 * DO NOT uncomment if you are running StandardFirmataEthernet on an Arduino
 * Leonardo,
 * Arduino Micro or other ATMega32u4-based board or you will not have enough
 * Flash and RAM
 * remaining to reliably run Firmata. Arduino Yun is okay because it doesn't
 * import the Ethernet
 * libraries.
 */
// In order to use software serial, you will need to compile this sketch with
// Arduino IDE v1.6.6 or higher. Hardware serial should work back to Arduino
// 1.0.
...
unsigned int samplingInterval =
    19;  // how often to sample analog inputs (in ms)
...
void readAndReportData(byte address,
                       int theRegister,
                       byte numBytes,
                       byte stopTX) {
...
/*
 * StandardFirmataEthernet communicates with Ethernet shields over SPI.
 * Therefore all
 * SPI pins must be set to IGNORE. Otherwise Firmata would break SPI
 * communication.
 * Additional pins may also need to be ignored depending on the particular board
 * or
 * shield in use.
 */
void ignorePins() {
  ...
  pinMode(PIN_TO_DIGITAL(53),
          OUTPUT);  // configure hardware SS as output on MEGA
#endif
...
void initTransport() {
  Ethernet.setCsPin(PE11);
  Ethernet.setRstPin(PE12);
#ifdef YUN_ETHERNET
  Bridge.begin();
#else
#ifdef local_ip
...
void loop() {
  ...
  // only necessary when using DHCP, ensures local IP is updated appropriately
  // if it changes
